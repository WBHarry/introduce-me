 ...
 */
function buildJS() {
  return gulp.src(`src/**/*.js`).pipe(gulp.dest(`dist/${moduleId}`));
}
 ...
 */
function buildJSMap() {
  return gulp.src(`src/**/*.js.map`).pipe(gulp.dest(`dist/${moduleId}`));
}
 ...
 */
function buildMJS() {
  return gulp.src(`src/**/*.mjs`).pipe(gulp.dest(`dist/${moduleId}`));
}
...
const linkUserData = async () => {
  ...
  try {
    if (
      fs.existsSync(path.resolve(`.`, `dist/${moduleId}`, `module.json`)) ||
